...
//        String filePah = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + context.getPackageName() + File.separator + dirName;
    ...
    fun copyFile(
        inputPath: String?, destination: File, extension: String
    ): String {
        val calendar = Calendar.getInstance()
        val filename =
...
//        a.setDuration((int) (initialHeight / v.getContext().getResources().getDisplayMetrics().density));
    ...
    fun getContrastColor(bgColor: Int): Int {
        return Color.rgb(
            255 - Color.red(bgColor), 255 - Color.green(bgColor), 255 - Color.blue(bgColor)
        )
    }
    ...
    fun convertTimeInAM_PM(hours: Int, minutes: Int): String {
        ...
        return StringBuilder().append(hour).append(':').append(min).append(" ").append(timeSet)
            .toString()
    }
    ...
    fun animateView(view: View, toVisibility: Int, toAlpha: Float, duration: Int) {
        ...
        view.visibility = View.VISIBLE
        view.animate().setDuration(duration.toLong()).alpha(if (show) toAlpha else 0f)
    ...
    fun formatDecimal(str: String, MAX_DECIMAL: Int): String? {
        ...
        val formatter = DecimalFormat(
            "#,###." + getDecimalPattern(str, MAX_DECIMAL), DecimalFormatSymbols(Locale.US)
        )
